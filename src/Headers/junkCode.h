//Code I wanted to save for reference later
//not ment to be used
void createWindow(char* arguments, void* tempHashMap){
  hash* hasher = malloc(sizeof(hash));
  hasher = (hash*)tempHashMap;
  printf("%ld\n", hasher->size);
  sleep(5);
  char* source = "gnome-terminal ";
  char* temp = malloc(strlen(source)*sizeof(char) + strlen(arguments));
  strcpy(temp, source);
  char* commands = strcat(temp, arguments);
  int exit_status = system(source);
}
